import request from './upload/upload.js';
...
$http.getQnToken = function(callback){
	...
	callback({
		visitPrefix: 'http://pic.shxhui.com/',
		token: 'X_cH7XZh-1ZckDrxXaLV5DguisLN66ieJI1b7eCV:ymHIj8scjzE9Wpad8L0SxX3ZxpU=:eyJzY29wZSI6ImZtMjAxNyIsIm1pbWVMaW1pdCI6ImltYWdlLyoiLCJkZWFkbGluZSI6MTU5NzIyNzM3MSwiZmlsZVR5cGUiOjB9',
		folderPath: ''
	});
...
$http.requestStart = function(options) {
	...
	// #ifdef APP-PLUS
	// 添加当前版本号
	if(store.state.vuex_version){
		options.header['version_code'] = store.state.vuex_version;
	}
	// #endif
	//请求前加入token
...
$http.dataFactory = async function(res) {
	...
	if (res.response.statusCode && res.response.statusCode == 200) {
		...
		//判断数据是否请求成功
		// if (httpData.success || httpData.code == 200) {    //一种接口格式
		if (httpData) {			//业务需求格式
			// return Promise.resolve(httpData.data);				 //一种接口格式
